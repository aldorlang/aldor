...
String emitJavaHeader = "//\n// Java generated by aldor from file %s\n//\n";

void
emitTheJava(EmitInfo finfo, JavaCode java)
{
	Foam foam;
	FileNameList fl;
	OStream      o;
	...
	srcfn	 = emitSrcFile(finfo);
	fn	 = emitFileName(finfo, FTYPENO_JAVA);

	emitInfoInUse(finfo, FTYPENO_JAVA) = true;
	fout	 = fileWrOpen(fn);

	fnstring = fnameUnparseStaticWithout(srcfn);
	fprintf(fout, emitJavaHeader, fnstring);

	o = ostreamNewFrFile(fout);
	ctxt = jcoPContextNew(o, false);
	jcoWrite(ctxt, java);
