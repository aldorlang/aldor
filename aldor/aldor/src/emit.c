...
emitDoneOptions(int argc, String *argv)
{
	...
	emitNeed[FTYPENO_INTERMED] = emitKeep[FTYPENO_INTERMED] |=
		!(emitDo[FTYPENO_INCLUDED] || emitDo[FTYPENO_CPP] ||
		  ...
		  emitDo[FTYPENO_AXLMAINC]  || emitDo[FTYPENO_JAVA] ||
		  emitDoRun || emitDoInterp);
...
void
emitTheJava(EmitInfo finfo, JavaCodeList javaFiles)
{
	emitInfoInUse(finfo, FTYPENO_JAVA) = true;

	while (javaFiles != listNil(JavaCode)) {
		emitOneJavaFile(finfo, car(javaFiles));
		javaFiles = cdr(javaFiles);
	}

	emitInfoInUse(finfo, FTYPENO_JAVA) = false;
	emitSetDone(FTYPENO_JAVA);
}


local void
emitOneJavaFile(EmitInfo finfo, JavaCode javaFile)
{
	...
	srcfn = emitSrcFile(finfo);
	fn   = emitJavaFileName(finfo, javaFile);

	fout = fileWrOpen(fn);
	ostream = ostreamNewFrFile(fout);
	ostreamPrintf(ostream, emitJavaHeader, fnameUnparseStaticWithout(srcfn));

	ctxt = jcoPContextNew(ostream, false);
	jcoWrite(ctxt, javaFile);
...
local FileName
emitJavaFileName(EmitInfo finfo, JavaCode javaFile)
{
	FileName javaFileName = emitFileName(finfo, FTYPENO_JAVA);
	String fileName = jcFileClassName(javaFile);
	String pkgName = strReplace(jcFilePackageName(javaFile), ".", "/");
	String destDir = strPrintf("%s/%s", fnameDir(javaFileName),
				   pkgName == NULL ? "" : pkgName);

	return fnameNew(destDir, fileName, FTYPE_JAVA);
}
