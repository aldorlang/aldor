...
jflowProg(Foam prog)
{
	...
	jflowDfDEBUG(dbOut, "JFlow: %pFoam\n", prog);
...
jflowDummyBlocksFind(JFInfo jfinfo, FlowGraph flog)
{
	...
	flogIter(flog, bb, {
		...
		if (i < ixN) continue;
		jflowDmDEBUG(dbOut, "Found dummy: Var: %d Block: %d\n",
			     cvn, bb->label);
		      ...
		      Bitv cloneSet, BitvClass class)
{
	...
	jflowDmDEBUG(dbOut, "Header: %pBBlock Exit: %d\n", bb, exit);
	...
	jflowDmDEBUG(dbOut, "Cloning %d blocks\n", bitvCount(class, cloned));
	...
	for (i=0; i <= bitvMax(class, cloned); i++) {
		...
		jflowDmDEBUG(dbOut, "cloned: (%s) %pBBlock to %pBBlock\n",
	...
	jflowDmDEBUG(dbOut, "Header: %pBBlock\n", bb);
			       ...
			       Dominators doms)
{
	...
	while (selLoops) {
		...
		jflowDmDEBUG(dbOut, "Looking at: %pBBlock, Candidate loop: %pLoop\n",
			     bb, selLoop);
...
jflowDummyTestRemove(JFInfo jfinfo, FlowGraph flog, BBlockList bbl)
{
	...
	jflowDmDEBUG(dbOut, "DummyTestRemove-Loops: %pLoopList\n", loops);
...
jflowDummyTestsRemove(JFInfo jfinfo, FlowGraph flog)
{
	...
	n = dflowFwdIterate(flog, DFLOW_Union, jflowDF_CUTOFF, &k, NULL);
	jflowDmDEBUG(dbOut, "DummyTestsRemove: %pFlowGraph\n", flog);
	if (DEBUG(jflowDf)) {
		fprintf(dbOut, n == 0 ? "Converged" : "Did not converge");
		fprintf(dbOut, " after %d iterations\n", k);
		flogPrint(dbOut, flog, true);
