...
fintInit(void)
{
	...
	*progInfoDEnv(prog) = POS_LEX_FMT;   /* null format */
	...
	fintUnitTape(mainUnit) = bufData(evalBuf);
...
loadMainUnit(Foam foam)
{
	...
	for (i = 1; j < n; i++) {
		...
		tape = bufData(buf);  /* here, because may change */
		ip = constPos;
	...
	fintUnitTape(mainUnit) = bufData(evalBuf);
	fintUnitTape(progUnit) = bufData(fintUnitBuffer(progUnit));
 ...
 */
	...
	case FOAM_BVal_SIntHashCombine:
		(void)fintEval(&expr1);
		(void)fintEval(&expr2);
		retDataObj->fiSInt = hashCombinePair(expr1.fiSInt, expr2.fiSInt);
		myType = FOAM_SInt;
		break;
