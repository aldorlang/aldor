...
#define fintDEBUG			if (false)
#define fintLinkDEBUG			if (false)
#define fintStoDEBUG			if (false)
#undef  assert
#define assert(x)
#define softAssert(x)
#define hardAssert(x)
...
#define fintDEBUG			if (fintDebug)
#define fintLinkDEBUG			if (fintLinkDebug)
#define fintStoDEBUG			if (fintStoDebug)
...
shDataObjAdd(AInt type, String id, int protocol, int globNum, FintUnit curUnit)
{
	...
	if (protocol == FOAM_Proto_Init)
		...
		if (found) {
			...
			fintLinkDEBUG {
				(void)fprintf(dbOut, "(Resolved foreign %s)\n",
					      fintForeignTable[n].string);
			}
		} else {
			fintLinkDEBUG {
				fprintf(dbOut, "Could not resolve: %s\n", id);
			}
		}
	}
