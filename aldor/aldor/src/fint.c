...
local Bool	fintTimings = true; /* Display timings in loop mode? */
local Bool 	fintConfirm = true;
local long 	fintMsgLimit = ABPP_UNCLIPPED;    /* Default no msg limit */
local long      fintExntraceMode = 0;    /* Default: no trace */
...
fintExecMainUnit(void)
{
	...
	if (fintMode == FINT_LOOP) {
    		oldCompFintBreakHandler =
			osSetBreakHandler(compFintBreakHandler);
		oldCompFintFaultHandler = 
			osSetFaultHandler(compFintFaultHandler);
	}
