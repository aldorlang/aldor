----------------------------- sal_llist.as -------------------------------------
--
-- sortedlist.as: A basic constructor for sorted lists.
--
...
\History{Manuel Bronstein}{24/3/2004}{fixed bugs 1357 and 1358 (remove!)}
\Usage{import from \this~T}
\Params{ {\em T} & \altype{PartiallyOrderedType} & the type of the entries\\ }
\Descr{\this(T) provides sorted lists whose entries belong 
to a partially ordered type. Duplicate entries are allowed.}
\begin{exports}
...
SortedList(T: PartiallyOrderedType):
	...
	remove!(t:T,x:%) : % == {
		...
		t < first(l) => x;
		t = first l => per rest l;
		-- m:Rep := empty;     -- MB: fix to bugs 1357 and 1358
		m := l;
		while (not empty?(l := rest l)) and not (t < first(l)) repeat {
			if t = first(l) then {
				assert(~empty? m);
