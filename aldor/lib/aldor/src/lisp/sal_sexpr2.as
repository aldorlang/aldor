...
== add 
    ...
    rest(cons: %): SExpression2 == rep(cons).cdr
    cons(car: SExpression2, cdr: SExpression2): % == per [car, cdr]

    setFirst!(cons: %, sx: SExpression2): () == rep(cons).car := sx
...
SExpression2: Join(InputType, OutputType, PrimitiveType) with
    sexpr: Symbol -> %
    sexpr: Integer -> %
    sexpr: String -> %
    sexpr: Cons -> %
...
== add 
    ...
    setRest!(sx, r: SExpression2): () == setRest!(rep(sx).CONS, r)
    ...
    local copyList(sx): (SExpression2, SExpression2) ==
        nil? sx => never
    ...
    <<(rdr: TextReader): % ==
        import from SExpressionReader2, Partial %
	retract read rdr

LStream2(T: Type): Category == with
    peek: % -> T
    next!: % -> ()
    hasNext?: % -> Boolean

TextLStream2: LStream2 Character with
    tstream: TextReader -> %;
== add
...
FnLStream2(T: Type): LStream2 T with
...
SExpressionReader2: with
   read: (TextReader) -> Partial SExpression2;
   readCased: (TextReader)  -> Partial SExpression2;
== add
    Token == Record(type: 'sym,escsym,number,str,ws,oparen,cparen,dot,quote,error,getref,setref', txt: String);
    import from Token
    import from CharSets2

    read(rdr: TextReader): Partial SExpression2 == read(rdr, false)
    readCased(rdr: TextReader): Partial SExpression2 == read(rdr, true)

    local read(rdr: TextReader, cased: Boolean): Partial SExpression2 ==
        import from TextLStream2
        import from FnLStream2 Token
        s := tstream rdr
        tokstrm := tstream((): Partial Token +->  readOneToken! s)
        sxMaybe: Partial SExpression2 := read(tokstrm, cased)
	sxMaybe
    ...
    local readOneToken!(s: TextLStream2): Partial Token ==
        import from Character
    ...
    local readString(s: TextLStream2): Token ==
    ...
    local readReference(s: TextLStream2): Token ==
        next! s
	text := ""
    ...
    local readWhitespace(s: TextLStream2): Token ==
        buffer: StringBuffer := new()
        writer: TextWriter := coerce buffer
        import from Character
        writer << peek s
    ...
    local readEscaped(s: TextLStream2): Token ==
        import from Character
        buffer: StringBuffer := new()
        writer: TextWriter := coerce buffer
	next! s
    ...
    local readBackslashEscaped(s: TextLStream2): Token ==
        next! s
        text := peek(s)::String
        next! s
	[escsym, text]

    local readNumber(s: TextLStream2): Token ==
        buffer: StringBuffer := new()
        writer: TextWriter := coerce buffer
    ...
    local readSymbol(s: TextLStream2): Token ==
        buffer: StringBuffer := new()
        writer: TextWriter := coerce buffer
    ...
    local read(s: FnLStream2 Token, cased: Boolean): Partial SExpression2 ==
        import from SExpression2, Symbol
	tbl: HashTable(String, SExpression2) := table()
	setref!(id: String, psx: Partial SExpression2): Partial SExpression2 ==
	    if not failed? psx then tbl.id := retract psx
	    psx
	getref(id: String): Partial SExpression2 == find(id, tbl)
        skipWhitespace!(): () ==
            while hasNext? s and peek(s).type = ws repeat
	        next! s

        readList(): Partial SExpression2 ==
	    not hasNext? s =>
	...
	readQuoted(): Partial SExpression2 ==
	    sx := read()
	    failed? sx => failed
	    [[sexpr(-"QUOTE"), retract sx]]
	    
        read(): Partial SExpression2 ==
	    import from Integer
            skipWhitespace!()
	    not hasNext? s =>
		stdout << "no next" << newline
...
# if 0
readOne(s: String): Partial SExpression2 ==
    import from SExpressionReader2
    sb: StringBuffer := new()
    sb::TextWriter << s
    read(sb::TextReader)

test(): () ==
    import from Partial SExpression2
    import from SExpression2
    import from Assert SExpression2
    import from Integer
    import from Symbol
