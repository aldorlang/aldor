...
testEvaluate(): () ==
    ...
    expr := _or(expression atom 1, _and(expression atom 2, expression atom 3));
    dnf := normalForm expr;
    stdout << "Normal " << expr << " ---> " << dnf << newline;
    for expr in allExprs repeat
        ...
        for i in 1..4@MachineInteger repeat
            stdout << "Test " << i << " " << expr << " ... " << dnf << newline;
            stdout << "Test " << ll.i  << " " << evaluate(Bit, valuation(i)) expr
                   << evaluate(Bit, valuation(i)) dnf << newline
